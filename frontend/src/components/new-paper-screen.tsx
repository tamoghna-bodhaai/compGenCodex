...
export function NewPaperScreen() {
  ...
  const [seedPickerKey, setSeedPickerKey] = useState<string | null>(null);
  ...
  const view = useMemo(() => {
    ...
    const planStates = creation.subtopicKeys.map((key, index) => {
      ...
      const validTypes = planPayload.question_types.every((item) => (typeAvailability[item.type] || 0) >= 1);
      const validDifficulties = planPayload.difficulty_distribution.every((item) => { const key = DIFFICULTIES.find(([, , difficulty]) => difficulty === item.difficulty)?.[0]; return !!key && (difficultyAvailability[key] || 0) >= 1; });
      const hasSelectedSeeds = plan.seedQuestionIds.length > 0;
      return { key, topic, subtopic, plan, hasSelectedSeeds, typeAvailability, difficultyAvailability, typeTotal, difficultyTotal, validTypes, validDifficulties };
    });
    const ready = Boolean(payload.title) && planStates.length > 0 && planStates.every((item) => item.typeTotal > 0 && item.typeTotal === item.difficultyTotal && (item.hasSelectedSeeds || (item.validTypes && item.validDifficulties)));
  ...
  return <section className={`${s.content} ${s.narrowContent}`}>
    <PageHeader eyebrow="Create paper" title="Build a precise paper request" description="Generate from one or more compatible seeds, or select exact source questions for a section." />
    <form className={s.creationForm} onSubmit={submit}>
      ...
      <section className={s.formSection}><div className={s.formSectionHead}><span>04</span><div><h2>Section plans</h2><p>Match question-type and difficulty totals for every section.</p></div></div><div className={s.planList}>{view.planStates.length ? view.planStates.map((item, index) => { const hasSelectedSeeds = item.hasSelectedSeeds; const valid = item.typeTotal > 0 && item.typeTotal === item.difficultyTotal && (hasSelectedSeeds || (item.validTypes && item.validDifficulties)); const coverageName = item.subtopic ? `${item.topic} › ${item.subtopic}` : item.topic; const hint = !item.typeTotal ? "Set at least one question." : item.typeTotal !== item.difficultyTotal ? `Type total (${item.typeTotal}) must equal difficulty total (${item.difficultyTotal}).` : hasSelectedSeeds ? `${item.typeTotal} variations will use all ${item.plan.seedQuestionIds.length} selected source question${item.plan.seedQuestionIds.length === 1 ? "" : "s"}; the primary source rotates per variation.` : !item.validTypes || !item.validDifficulties ? "Each chosen type/difficulty needs at least one compatible seed, or select source questions below." : `${item.typeTotal} questions · available compatible seeds are used dynamically.`; return <article className={s.planCard} key={item.key}><div className={s.planTitle} onDoubleClick={() => setSeedPickerKey(item.key)}><span>Section {view.planStates.length > 1 ? index + 1 : ""}</span><strong>{coverageName}</strong></div><Field label="Section title"><Input value={item.plan.sectionTitle} placeholder={coverageName} onChange={(event) => patchPlan(item.key, { sectionTitle: event.target.value })} /></Field><section className={s.seedSourcePanel}><div><strong>Source questions for variations</strong><p>{hasSelectedSeeds ? `${item.plan.seedQuestionIds.length} selected · every variation uses the full source set, with a rotating primary source.` : "Optional: select one or more exact bank questions to use together as variation sources."}</p></div><Button size="small" onClick={() => setSeedPickerKey(item.key)}>{hasSelectedSeeds ? "Edit sources" : "Choose questions"}</Button></section><div className={s.twoCol}><Field label="Questions by type"><div className={s.countGrid}>{QUESTION_TYPES.map(([type, label]) => <label className={`${s.countPicker} ${!item.typeAvailability[type] && !hasSelectedSeeds ? s.unavailable : ""}`} key={type}><span>{label}<small>{item.typeAvailability[type]} seeds</small></span><Input type="number" min={0} disabled={!item.typeAvailability[type] && !hasSelectedSeeds} value={item.plan.questionCounts[type] || 0} onChange={(event) => setPlanCount(item.key, type, Number(event.target.value))} /></label>)}</div></Field><Field label="Questions by difficulty"><div className={s.countGrid}>{DIFFICULTIES.map(([key, label]) => <label className={`${s.countPicker} ${!item.difficultyAvailability[key] && !hasSelectedSeeds ? s.unavailable : ""}`} key={key}><span>{label}<small>{item.difficultyAvailability[key]} seeds</small></span><Input type="number" min={0} disabled={!item.difficultyAvailability[key] && !hasSelectedSeeds} value={item.plan.difficultyCounts[key] || 0} onChange={(event) => setDifficultyCount(item.key, key, Number(event.target.value))} /></label>)}</div></Field></div><div className={s.twoCol}><Field label="Variation mode"><Select value={item.plan.generation_mode} onChange={(event) => patchPlan(item.key, { generation_mode: event.target.value as CreationPlan["generation_mode"] })}><option value="structural_variation">Structural variation</option><option value="concept_variation">Concept variation</option></Select></Field><Field label="Variation strength"><Select value={item.plan.variation_strength} onChange={(event) => patchPlan(item.key, { variation_strength: event.target.value as CreationPlan["variation_strength"] })}><option value="close">Close to source</option><option value="balanced">Balanced</option><option value="high">High variation</option></Select></Field></div><p className={valid ? s.validText : s.warningText}>{hint}</p></article>; }) : <div className={s.inlineEmpty}>Select subtopics to configure per-section counts.</div>}</div></section>
    ...
    {seedPickerKey && (() => { const [topic, subtopic] = splitSubtopicKey(seedPickerKey); const plan = creation.plans[seedPickerKey] || defaultPlan(); return <QuestionBrowserDialog open selection={{ exam: creation.exam, subject: creation.subject, topic, ...(subtopic ? { subtopic } : {}) }} chapters={creation.chapters} title="Choose source questions" selectable selectedIds={plan.seedQuestionIds} onConfirm={(seedQuestionIds) => { patchPlan(seedPickerKey, { seedQuestionIds }); setSeedPickerKey(null); }} onClose={() => setSeedPickerKey(null)} />; })()}
  </section>;
}
