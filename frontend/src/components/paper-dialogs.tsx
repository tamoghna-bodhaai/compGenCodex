...
export function PaperDialogs({ kind, paper, comparisonQuestion, onClose, onChanged, toast }: { kind: EditorDialog; paper: Paper; comparisonQuestion?: PaperQuestion | null; onClose: () => void; onChanged: () => Promise<void>; toast: (message: string, tone?: "default" | "error") => void }) {
  ...
  const comparisonQuestionId = comparisonQuestion?.id;

  useEffect(() => {
    const controller = new AbortController();
    if (kind === "source" && !sourceQuestions.length) api.questions(new URLSearchParams({ limit: "100" }), controller.signal).then((result) => setSourceQuestions(result.items)).catch((error) => { if (!controller.signal.aborted) toast(error instanceof Error ? error.message : "Couldn’t load seed questions.", "error"); });
    if (kind === "seed-comparison" && comparisonQuestionId) {
      setComparison(null); setComparisonError(""); setSelectedSeedIndex(0);
      api.paperQuestionSeeds(paper.id, comparisonQuestionId, controller.signal).then(setComparison).catch((error) => { if (!controller.signal.aborted) setComparisonError(error instanceof Error ? error.message : "Couldn’t load the referenced seed questions."); });
    }
    if (kind === "comparison") {
      setPaperComparison(null); setComparisonError(""); setSelectedPaperComparisonIndex(0);
      api.paperComparison(paper.id, controller.signal).then(setPaperComparison).catch((error) => { if (!controller.signal.aborted) setComparisonError(error instanceof Error ? error.message : "Couldn’t load the reference comparison."); });
    }
    return () => controller.abort();
  }, [kind, paper.id, comparisonQuestionId, sourceQuestions.length, toast]);
  ...
  const primarySeedQuestionId = typeof comparison?.generation_metadata.primary_seed_question_id === "string" ? comparison.generation_metadata.primary_seed_question_id : null;
