...
export function PaperEditorScreen({ paperId }: { paperId: string }) {
  ...
  useEffect(() => { const controller = new AbortController(); api.paperExports(paperId, controller.signal).then((result) => setExportHistory(result.items)).catch((caught) => { if (!controller.signal.aborted) toast(caught instanceof Error ? caught.message : "Couldn’t load saved exports.", "error"); }); return () => controller.abort(); }, [paperId, toast]);
