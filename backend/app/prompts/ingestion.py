...
When a page image is attached, mark `diagram_required` only if a visual diagram is necessary to solve the question. Supply `diagram_bbox` as normalized [x, y, width, height] on the attached source page, tightly covering the diagram, and give an exact description plus an answer-independent render specification (labels, geometry and relationships). Never mark decorative images, logos, tables, or graphs unrelated to a question."""
